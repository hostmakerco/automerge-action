...
const { updateMerge, updateRebase } = require("./update");
...
async function updateAndMergePullRequest(context, pullRequest) {
  ...
  await tmpdir(path =>
    updateRebase(context, path, cloneUrl, pullRequest)
  );
  const head = await tmpdir(path =>
    updateMerge(context, path, cloneUrl, pullRequest)
  );
...
async function handleBranchUpdate(context, eventName, event) {
  ...
  for (const pullRequest of pullRequests) {
    ...
    try {
      await tmpdir(path => updateRebase(context, path, cloneUrl, pullRequest));
      updated++;
    } catch (e) {
      if (e instanceof NeutralExitError) {
        logger.trace("PR update has been skipped.");
